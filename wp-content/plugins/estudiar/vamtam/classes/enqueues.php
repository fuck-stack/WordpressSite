...
class VamtamEnqueues {
	...
	public static function admin_scripts() {
		...
		wp_localize_script(
			'vamtam-admin', 'VAMTAM_ADMIN', array(
				'addNewIcon' => esc_html__( 'Add New Icon', 'estudiar' ),
				'iconName'   => esc_html__( 'Icon', 'estudiar' ),
				'iconText'   => esc_html__( 'Text', 'estudiar' ),
				'iconLink'   => esc_html__( 'Link', 'estudiar' ),
				'iconChange' => esc_html__( 'Change', 'estudiar' ),
				'fonts'      => $GLOBALS['vamtam_fonts'],
				'home_url'   => home_url(),
				'elementsTxt' => esc_html__( 'Enter your Envato Elements Token.', 'estudiar' ),
				'tfPcTxt' => esc_html__( 'Enter your purchase code from ThemeForest to receive theme updates and support.', 'estudiar' ),
				'unRegPcTxt' => esc_html__( 'You are about to unregister your purchase code. Are you sure you want to continue?', 'estudiar' ),
				'unRegTokenTxt' => esc_html__( 'You are about to unregister your Envato Elements Token. Are you sure you want to continue?', 'estudiar' ),
		...
		if ( $pagenow === 'update-core.php' ) {
			...
			<script type="text/javascript">
				document.addEventListener( 'DOMContentLoaded', function() {
					...
					if ( ! $valid_pc ) {
						$update_notice  = '<br id="vamtam-notice-br" />' . __( 'Note: Only Envato Market clients with a valid purchase code are entitled to automatic updates. Envato Elements clients must use FTP.', 'estudiar' );
						$changelog_link = '<span><a href="https://vamtam.com/changelog" target="_blank">' . __('View Changelog', 'estudiar') . '</a></span>';
	...
	public static function styles() {
		...
		$fallback = vamtam_extra_features() ? '' : '-fallback';
		...
		wp_register_style( 'vamtam-wc-cart-checkout', VAMTAM_ASSETS_URI . "css/dist/woocommerce/cart-checkout{$fallback}.css" , array( 'vamtam-front-all' ), $cache_timestamp );
		wp_register_style( 'vamtam-wc-styles', VAMTAM_ASSETS_URI . "css/dist/woocommerce/main{$fallback}.css" , array( 'vamtam-front-all' ), $cache_timestamp );
		...
		if ( vamtam_has_woocommerce() ) {
			...
			wp_enqueue_style( 'vamtam-theme-wc-small-screen', $url_prefix . "wc-small-screen{$fallback}.css", array( 'vamtam-front-all' ), $cache_timestamp, $wc_small_screen_media );
		}
