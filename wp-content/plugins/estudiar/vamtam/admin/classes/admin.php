...
class VamtamAdmin {
	...
	public static function update_warning() {
		if ( did_action( 'load-update-core.php' ) ) {
			...
			esc_html_e( 'Hey, just a polite reminder that if you update WordPress you will also need to update your theme and plugins.', 'estudiar' );
			echo '</div></div>';
		}
		...
		if ( did_action( 'load-update-core.php' ) || did_action( 'load-themes.php' ) ) {
			...
			esc_html_e( 'VamTam theme resources: ', 'estudiar' );
			echo '</strong>';
			echo '<a href="https://vamtam.com/child-themes" target="_blank">';
			esc_html_e( 'Sample child themes', 'estudiar' );
			echo '</a>; ';
			echo '<a href="https://vamtam.com/changelog" target="_blank">';
			esc_html_e( 'Changelog', 'estudiar' );
			echo '</a>';
			echo '</div></div>';
		}
	}
	...
	public static function icons_selector() {
		...
		<div class="vamtam-config-icons-selector hidden">
			<input type="search" placeholder="<?php esc_attr_e( 'Filter icons', 'estudiar' ) ?>" class="icons-filter"/>
	...
	public static function theme_upgrade_no_pkg( $reply, $package, $upgrader ) {
		if ( isset( $upgrader->skin->theme_info ) && false !== $upgrader->skin->theme_info ) {
			...
			if ( $theme_slug === VAMTAM_THEME_SLUG && empty( $package ) ) {
				return new WP_Error( 'no_package', __( 'Only Envato Market clients with a valid purchase code are entitled to automatic updates. Envato Elements clients must use FTP.', 'estudiar' ) );
			}
		}
