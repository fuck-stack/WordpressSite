...
function vamtam_register_required_plugins() {
	...
	$plugins = array(
		...
		array(
			'name'     => esc_html__( 'Elementor', 'estudiar' ),
			'slug'     => 'elementor',
			'required' => true,
			'category' => 'required',
			'version'  => '2.7.4',
		...
		array(
			'name'     => esc_html__( 'Vamtam Elementor Integration', 'estudiar' ),
		...
		array(
			'name'     => esc_html__( 'Vamtam Importers (E)', 'estudiar' ),
		...
		array(
			'name'     => esc_html__( 'Wordpress SEO', 'estudiar' ),
			'slug'     => 'wordpress-seo',
			'required' => false,
		...
		array(
			'name'     => esc_html__( 'Ally - Web Accessibility & Usability', 'estudiar' ),
		...
		array(
			'name'     => esc_html__( 'Really Simple SSL', 'estudiar' ),
		...
		array(
			'name'     => esc_html__( 'Loco Translate', 'estudiar' ),
			'slug'     => 'loco-translate',
			'required' => false,
			'category' => 'recommended',
		),
		...
		array(
			'name'     => esc_html__( 'UpDraftPlus', 'estudiar' ),
			'slug'     => 'updraftplus',
			'required' => false,
			'category' => 'recommended',
		),
		...
		array(
			'name'     => esc_html__( 'LiteSpeed Cache', 'estudiar' ),
		...
		array(
			'name'     => esc_html__( 'Limit Login Attempts Reloaded', 'estudiar' ),
		...
		array(
			'name'     => esc_html__( 'Wordfence', 'estudiar' ),
			'slug'     => 'wordfence',
			'required' => false,
			'category' => 'recommended',
		),
		...
		array(
			'name'     => esc_html__( 'CookieYes | GDPR Cookie Consent & Compliance Notice (CCPA Ready)', 'estudiar' ),
...
function vamtam_tgmpa_table_columns( $columns ) {
	...
	$columns = array(
		...
		'img'         => '',
		'plugin'      => esc_html__( 'Name', 'estudiar' ),
		'description' => esc_html__( 'Description', 'estudiar' ),
		'status'      => esc_html__( 'Status', 'estudiar' ),
		'version'     => esc_html__( 'Version', 'estudiar' ),
	);
	return $columns;
}
...
function vamtam_tgmpa_table_data_item( $item, $plugin )
{
	...
	if( isset( $plugin_data['Description'] ) ) {
		...
		$item['description'] =  esc_html__( 'A simple WordPress Plugin.', 'estudiar' );
	}
	if ( strpos( $plugin['slug'], 'estudiar' ) !== false ) {
		$item['description'] =  esc_html__( 'Theme Exclusive.', 'estudiar' );
	}
...
function vamtam_tgmpa_admin_menu_args( $args ) {
	$args['page_title'] = esc_html__( 'Install Plugins', 'estudiar' );
